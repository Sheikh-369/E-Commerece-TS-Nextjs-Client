...
import { fetchCartItems } from "@/lib/store/cart/cart-slice";
import { createAnOrder } from "@/lib/store/check-out/check-out-slice";
import { IOrderData, PaymentMethod } from "@/lib/store/check-out/check-out-slice-type";
import { ICartItem } from "@/lib/store/cart/cart-slice-type";
import Image from "next/image";
import { useSearchParams } from "next/navigation";
            ...
            <div className="px-6 py-8 md:overflow-auto md:h-screen">
              ...
              <div>
                <ul className="text-slate-500 font-medium space-y-4">
                  <li className="flex flex-wrap gap-4 text-sm">
                    Subtotal{" "}
                    <span className="ml-auto font-semibold text-slate-900">Rs. {subtotal.toFixed(2)}</span>
                  </li>
                  <li className="flex flex-wrap gap-4 text-sm">
                    Shipping{" "}
                    <span className="ml-auto font-semibold text-slate-900">Rs. {shipping.toFixed(2)}</span>
                  </li>
                  <li className="flex flex-wrap gap-4 text-sm">
                    Tax{" "}
                    <span className="ml-auto font-semibold text-slate-900">Rs. {taxes.toFixed(2)}</span>
                  </li>
                  <hr className="border-slate-300" />
                  <li className="flex flex-wrap gap-4 text-[15px] font-semibold text-slate-900">
                    Total <span className="ml-auto">Rs. {total.toFixed(2)}</span>
        ...
        <div className="max-w-4xl w-full h-max rounded-md px-4 py-8 max-md:-order-1">
          <form onSubmit={handleOrderDataSubmission}>
            <div>
              ...
              <div className="grid lg:grid-cols-2 gap-y-6 gap-x-4">
                {["firstName","lastName","email","phoneNumber","province","district","city","tole"].map((field) => (
                  <div key={field}>
                    <label className="text-sm text-slate-900 font-medium block mb-2">{field.replace(/([A-Z])/g, " $1")}</label>
                    <input
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <div className="bg-gray-100 p-4 rounded-md border border-gray-300 max-w-sm">
                  <div className="flex items-center">
                    <input
                      ...
                      value={PaymentMethod.COD}
                      checked={orderData.paymentMethod === PaymentMethod.COD}
                      onChange={(e) => setOrderData({ ...orderData, paymentMethod: e.target.value as PaymentMethod })}
                      className="w-5 h-5 cursor-pointer"
                      id="cod"
                    />
                    <label htmlFor="cod" className="ml-4 flex items-center gap-2 cursor-pointer">
                      <span className="text-sm font-medium text-slate-700">Cash on Delivery</span>
                      <Image src="/check-out/COD.jpg" alt="COD" height={54} width={54} className="w-8 h-8" />
                    </label>
                  </div>
                  <p className="mt-4 text-sm text-slate-500 font-medium">Pay with cash when your order is delivered.</p>
                </div>
                ...
                <div className="bg-gray-100 p-4 rounded-md border border-gray-300 max-w-sm">
                  <div className="flex items-center">
                    ...
                    <label htmlFor="khalti" className="ml-4 flex items-center gap-2 cursor-pointer">
                      <span className="text-sm font-medium text-slate-700">Khalti</span>
                      <Image src="/check-out/Khalti.jpg" alt="Khalti" height={50} width={50} className="w-12 h-8 object-contain" />
                    </label>
                  </div>
                  <p className="mt-4 text-sm text-slate-500 font-medium">Pay securely using your Khalti wallet.</p>
                </div>
              </div>
            </div>
